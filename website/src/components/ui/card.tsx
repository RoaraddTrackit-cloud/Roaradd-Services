...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "rounded-2xl border border-white/5 bg-card/40 text-card-foreground shadow-xl backdrop-blur-sm transition-all duration-300 hover:bg-card/60 hover:border-white/10",
        className
      )}
      {...props}
    />
